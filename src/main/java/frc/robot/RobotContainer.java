...
public class RobotContainer {
  ...
  private final PhotonCamera noteCamera = new PhotonCamera(cameraConstants.NOTE_CAMERA);
  private final PhotonCamera tagCamera = new PhotonCamera(cameraConstants.TAG_CAMERA);
  ...
  public RobotContainer() {
    ...
    m_driverController.leftBumper().onTrue(
        new RunCommand(
            () -> m_elevatorSubsystem.setSetpoint(Constants.elevatorConstants.AMP_ELEVATOR_EXTENSION_DISTANCE),
            m_elevatorSubsystem)
            .andThen(
                new RunCommand(() -> m_cannonMotorSubsystem.setCannonPower(Constants.cannonConstants.AMP_FIRING_POWER),
                    m_cannonMotorSubsystem))
            .andThen(
                new WaitCommand(2))
            .andThen(
